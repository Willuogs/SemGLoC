...
def ensemble(ds, items):
    if 'ntu120' in ds:
        num_class=120
        if 'xsub' in ds:
            npz_data = np.load('/mnt/D/znmd_lgs/ntu120/NTU120_CSub.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
        elif 'xset' in ds:
            npz_data = np.load('/mnt/D/znmd_lgs/ntu120/NTU120_CSet.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
    elif 'ntu' in ds:
        num_class=60
        if 'xsub' in ds:
            npz_data = np.load('/mnt/D/znmd_lgs/ntu/NTU60_CS.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
        elif 'xview' in ds:
            npz_data = np.load('/mnt/D/znmd_lgs/ntu/NTU60_CV.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
    elif 'UCLA' in ds:
        num_class=10
        # npz_data = np.load('./data/ntu/NTU60_CS.npz')
        # label = np.where(npz_data['y_test'] > 0)[1]
        with open('/mnt/D/znmd_lgs/NW-UCLA/val_label.pkl', 'rb') as f:
            d = pickle.load(f)
        label = []
        for item in d:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--motion_ckpts', nargs='*', default=[],
                    help='Directory containing "epoch1_test_score.pkl" for motion eval results (optional)')
    ...
    for ckpt in arg.position_ckpts:
        ...
        #item.append((ckpt, joint_weight if "joint" in ckpt else bone_weight))
    if arg.motion_ckpts:
